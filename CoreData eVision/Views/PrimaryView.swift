...
//            ImagePicker(sourceType: .camera, recognizeText: <#T##(UIImage) -> Void#>)
...
struct PrimaryView: View {
    @Environment(\.managedObjectContext) private var viewContext
    @State private var selectedTab: Tab = .first
    @State private var showingImagePicker = false // aggiungi questo stato
    ...
    var body: some View {
        VStack {
            ...
            CustomTabView(selectedTab: $selectedTab, showingCameraSheet: $showingImagePicker) // passa lo stato a CustomTabView
                .frame(height: 25)
                .padding(.top)
        }
